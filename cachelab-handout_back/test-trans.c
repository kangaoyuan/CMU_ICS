...
/*
 * eval_perf - Evaluate the performance of the registered transpose
 * functions
 */
void eval_perf(unsigned int s, unsigned int E, unsigned int b)
{
    ...
    for (i=0; i<func_counter; i++) {
        ...
        if (0!=flag) {
            printf("Validation error at function %d! Run ./tracegen -M %d "
                   "-N %d -F %d for details.\nSkipping performance "
                   "evaluation for this function.\n",
                   flag - 1, M, N, i);
            continue;
        }
        ...
        char cmd[255];
        sprintf(cmd,
                "./csim-ref -s %u -E %u -b %u -t trace.f%d > /dev/null", s,
                E, b, i);
        system(cmd);
...
int main(int argc, char* argv[])
{
    ...
    if (results.funcid == -1) {
        ...
        printf("Error: Please ensure that description field is exactly "
               "\"%s\"\n",
               SUBMIT_DESCRIPTION);
