 ...
 * allocated on the heap. Use in conjunction with the ./mem_valgrind.sh
...
int main() {
    ...
    int* arr[NUM_ARRS];
    // Iterate through array and allocate an array of ints for each int
    // pointer
    for (int i = 0; i < NUM_ARRS; i++) {
        // Note the explicit cast to int *
        arr[i] = malloc(BUF_SIZE * sizeof(int));
