...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) {
    ...
    stack_foo.bar = 3;
    stack_foo.baz = 4; // Always use "." for accessing a struct instance.

    // now a struct foo is allocated on the heap, so we have a
    // sturct *pointer to the struct instance. Accessing elements of a
    // struct from a pointer is slightly different.
