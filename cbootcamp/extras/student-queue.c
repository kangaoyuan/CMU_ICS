...
/*
    @brief Creates the initial queue by creating two encompassing blocks and
...
int enqueue(boundary* head, char* id, void* data, int size) {
    ...
    // TODO: Are all the functions you used safe? Is there a better way to
    // implement this? Feel free to modify the function header to whatever
    // you want to do it, if you have time.
...
/*
    ...
    @param return_data - buffer pre allocated by user that is large enough
   for data
    @param size - Amount of bytes that the user expects to be returned in
   buffer
    @return Status Value - 0 for success, non-zero for error
*/
int dequeue(boundary* head, void* return_data, int size) {
    ...
    // TODO: Make sure to clean up any resources that were allocated by this
    // library!
...
/*
    @brief Internal function that adds node to the queue by actually
...
/*
    @brief Internal helper function that frees all allocated parts of the
   nodes
