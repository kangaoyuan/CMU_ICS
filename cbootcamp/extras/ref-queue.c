...
int enqueue(boundary* head, char* id, void* data, int size) {
    // If the queue is malformed, or somehow a request violates the
    // invariants, return with error
    if (head == NULL)
        return -1;
    ...
    // Add individual entry to the queue while checking special case that
    // the new node is the first into the queue
