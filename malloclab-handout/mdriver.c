...
/* Returns true if p is ALIGNMENT-byte aligned, which is how to judge alignment. */
...
static int errors =
    0;             /* number of errs found when running student malloc */
...
int main(int argc, char** argv) {
    ...
    /*
     * Read and interpret the command line arguments, EOF == -1.
     */
    ...
    if (team_check) {
        ...
        if (!strcmp(team.teamname, "")) {
            printf("ERROR: Please provide the information about your team "
                   "in mm.c.\n");
            exit(1);
        } else
            printf("Team Name:%s\n", team.teamname);
        ...
        if (((*team.name2 != '\0') && (*team.id2 == '\0')) ||
            ...
            printf("ERROR.  You must fill in all or none of the team "
                   "member 2 ID fields!\n");
            exit(1);
    ...
    for (i = 0; i < num_tracefiles; i++) {
        ...
        if (mm_stats[i].valid) {
            printf("tracefile %d eval_mm_valid success\n", i);
            if (verbose > 1)
                printf("efficiency, ");
...
static trace_t* read_trace(char* tracedir, char* filename) {
    ...
    if ((trace->ops = (traceop_t*)malloc(trace->num_ops *
                                         sizeof(traceop_t))) == NULL)
