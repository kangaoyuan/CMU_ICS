...
{
    // Each thread has its own independent errno value
    fprintf(stderr, "%s: %s\n", msg, strerror(errno));
    exit(0);
}
...
// parent process returning val > 0, child process returning val == 0
pid_t Fork(void) {
    pid_t pid;
...
// if the process ID (PID) of the calling process is equal to its process group ID (PGID), it is the leader of its process group, and it is not allowed to change its process group via setpgid().
...
/*
 * The process that calls setsid() must not be the leader of any existing session.
 * when a process successfully calls setsid(), it creates both a new session and a new process group at the same time.
 * */
...
void Getnameinfo(const struct sockaddr* sa, socklen_t salen, char* host,
    ...
    if ((rc = getnameinfo(sa, salen, host, hostlen, serv, servlen,
                          flags)) != 0)
